...
public class AppointmentsService : IAppointmentsService
{
    ...
    public async Task<AppointmentDetailsDto> CreateAppointmentAsync(CreateAppointmentRequestDto dto)
    {
        ...
        {
            throw new AppointmentException(StatusCodes.Status400BadRequest, "Appointment date cannot be in the past.");
        }
        ...
        var idAppointment = await command.ExecuteScalarAsync() ??
            throw new AppointmentException(StatusCodes.Status500InternalServerError, "Failed to create appointment.");
    ...
    public async Task<AppointmentDetailsDto> UpdateAppointmentAsync(int idAppointment, UpdateAppointmentRequestDto dto)
    {
        ...
        if (existing == null)
        {
            throw new AppointmentException(StatusCodes.Status404NotFound, "Appointment does not exist.");
        }

        if (existing.Status == "Completed" && existing.AppointmentDate != dto.AppointmentDate)
        {
            throw new AppointmentException(StatusCodes.Status400BadRequest, "Cannot change the date of a completed appointment.");
        }
        ...
        return updatedAppointment ??
               throw new AppointmentException(StatusCodes.Status500InternalServerError, "Failed to update appointment.");
    }
    ...
    public async Task DeleteAppointmentAsync(int idAppointment)
    {
        ...
        {
            throw new AppointmentException(StatusCodes.Status409Conflict, "Cannot delete a completed appointment.");
        }
    ...
    private async Task ValidatePatient(SqlConnection connection, int idPatient)
    {
        ...
        if (value == null)
        {
            throw new AppointmentException(StatusCodes.Status400BadRequest, "Patient not found.");
        }
        if (!Convert.ToBoolean(value)) {
            throw new AppointmentException(StatusCodes.Status404NotFound, "Patient inactive.");
        }
    }
    ...
    private async Task ValidateDoctor(SqlConnection connection, int idDoctor)
    {
        ...
        if (value == null)
        {
            throw new AppointmentException(StatusCodes.Status400BadRequest, "Doctor does not exist.");
        }
        if (!Convert.ToBoolean(value)) {
    ...
    private async Task ValidateDate(SqlConnection connection, int idDoctor, DateTime date, int? idAppointment)
    {
        ...
        {
            throw new AppointmentException(StatusCodes.Status409Conflict, "Doctor is not available at the specified date and time.");
        }
    }
}